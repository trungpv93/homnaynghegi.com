...
CREATE TABLE urls (
  ...
  url_author    text NOT NULL DEFAULT '',
  url_date      timestamptz ,
  url_description text NOT NULL DEFAULT '',
  url_image     text NOT NULL DEFAULT '',
  url_publisher text NOT NULL DEFAULT '',
  ...
  reshared_url  int NOT NULL DEFAULT 0,
...
-- Speed up user_id FK joins
CREATE INDEX urls__user_id ON urls (user_id);
...
CREATE TABLE urlMeta (
  id            serial PRIMARY KEY,
  url_id        int  NULL REFERENCES urls(id),
  label         text NOT NULL,
  value         text NOT NULL DEFAULT ''
);

-- Speed up user_id FK joins
CREATE INDEX urlMeta__url_id ON urlMeta (url_id);
